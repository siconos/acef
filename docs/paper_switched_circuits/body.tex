...
When the circuit is simple and of low sizes in terms of number of state variables, it is sometimes possible to write the DAE as an ODE and performs explicit substitution of $X$ by $y$ and $\lambda$ in the formulation~(\ref{eq:71-discrete}). If the cone is also simply defined by a positive orthant, we arrive then a standard LCP \cite{denoyelle2006}. Unfortunately, the LCP formulation is not amenable for more complicated case  where an automatic circuit equation formulation (see the next section) is used.  
...
\end{array}}
\end{equation}
where $x \in \RR^n$ is corresponds to the current in the inductive branches and the voltages in the capacitive branches $z\in \RR^p$ collects all the node potentials, the current in the voltage--defined and non smooth branches and the currents in a subset of the capacitive branches. The choice and the construction of the latter subset of branches ids described in details in \cite{brevet}.  The automatic circuit equation formulation starts from the MNA and it  adds some unknowns to get a semi-explicit system, and replaces the constitutive equations of the non-smooth components by the corresponding inclusion rule. Starting from (\ref{eq:SEDGE}), the numerical algorithm as explained in \ref{Sec:ONSNP-solvers} is used in a similar manner on the time--discretized system,
\begin{equation}
...
The implementation is object-oriented and mainly on C++ and  for each electrical component and group of equations and inclusions in (\ref{eq:71-discrete}), a corresponding instance of a class is built. The system are updated in memory at each iteration by the stamp method of each component. In the linear case, theses methods are called only once, in the non-linear case they may be called at any time to update the system. The open-source platform is under GPL license and can be freely used. The equation generator is under private license and can be obtained freely on demand for an academic use.  
...
\end{figure}
\begin{ndrva}
  Add the current and the ramp on this Figure.
\end{ndrva}
...
PLECS simulation perform only 168038 steps comparing the $4.10^{6}$ perform during the SICONOS
