...
The time step has been fixed to $0.1 \micro \second$. The figure \ref{fig:SICONOS_SIMU_CS} depicts
...
figure \ref{fig:SICONOS_SIMU_CS}. Indeed, the linearization performed at each Newton-Raphson
...
OSNSP solver gives a correct solution. We have used indifferently PATH and SEMISMOOTH methods.
...
simulation, some messages like 'Newton no-convergence' appear. The figure \ref{fig:ELDO_SIMU_CS}
...
solvers used are PATH, and a semi-smooth Newton method based on the Fischer-Bursmeister
reformulation\cite{Deluca.ea1996} that is our own implementation.
...
The simulation has been tested with many parameters values. The robustness of the non-smooth
