...
{SICONOS simulation results, buck converter with a load resistor\protect\newline
steady state with $L~=~4~\mu H$, $C~=~10~\mu F$ , $R_{11}~=~10~K\Omega$, $R_{21}~=~8~M\Omega$, $C_{11}~=~10~pF$
%\protect\newline
...
to the $52~s$ of the SICONOS simulation, \textbf{obtained independently from these components values}.
The CPU time is therefore from~2.6 to~7.9 times larger than with SICONOS.
This demonstrates
...
{PLECS simulation results, buck converter with a load resistor\protect\newline
$L~=~4~\mu H$, $C~=~10~\mu F$ , $R_{11}~=~10~K\Omega$, $R_{21}~=~8~M\Omega$, $C_{11}~=~10~pF$
%\protect\newline
exhibiting a sliding mode}
...
This yields the same kind of affine complementarity system as
the single buck converter. Matrix~D belongs again to the P0-class.\\
...
\\
\\
\textbf{The simulation of the buck with a 2 inverters chain takes 441~s on a Pentium~4 clocked at 3~GHz.}
\\
...
\begin{itemize}
\item as explained in section \ref{section-simu-siconos-buckinv}, the SICONOS run time
will benefit from the use of sparse matrix techniques. It is expected that this will significantly
reduce the time of computation, since there are many calculations in the SICONOS engine that
could benefit from adding a specific sparse solver.
...
solvers will very likely reduce the CPU time without impairing the accuracy.
...
the current SICONOS version. This probably comes mainly from the use of sparse matrices and various other
implementation tricks which are not present in SICONOS, which is a software package whose development started
few years ago in an academic context.
...
\hline
Buck converter & 52 s & 356 s  & 135 s to 410 s \\
load : resistor & {\small slightly approximate } & {\small precise } & {\small very approximate } \\
\hline
Buck converter & 110 s & 441 s & 4 hours 8 min \\
