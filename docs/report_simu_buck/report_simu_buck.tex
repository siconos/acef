...
linked to a compiled simplified model would require less time than with a MOS level 3 model and that computation
involves operations other than Newton-Raphson algorithm.
\end{description}
...
(see a comparison of the PWL comparator as used in SICONOS simulations with the SPICE one on figure \ref{fig-comparator-models}).
\end{itemize}

\begin{figure}[hbtp]
\begin{center}
\includegraphics[scale=1.2,angle=0]{comparators.eps}
\end{center}
...
\hline
NGSPICE & simple  & 4000000 & 8024814 & 632 & 370\\
NGSPICE & level 3 & 4000000 & 8302904 & 383 & \\
\hline
SMASH   & simple  & 4000404 & 8073070 & 230 & 178\\
SMASH   & level 3 & 4000313 & 8052858 & 178 & \\
\hline
ELDO    & simple  & 4000000 & 4547579 & 388 & 354\\
ELDO    & level 3 & 4000000 & 4658412 & 363 & \\
\hline
\end{tabular}
\begin{tabular}{|l|c|r|r|r|r|}
\hline
...
NGSPICE & level 3 & failed & failed & failed & \\
\hline
SMASH   & simple  & 4000252 & 8239697 & 234 & 180\\
SMASH   & level 3 & 4000121 & 8191656 & 179 & \\
\hline
ELDO    & simple  & 4000000 & 5861226 & 438 & 361\\
ELDO    & level 3 & 4000000 & 6062189 & 374 & \\
\hline
\end{tabular}
\\
\\
...
SICONOS and NGSPICE simplified model results are very close.
...
\clearpage
Figure \ref{fig-compar-Sah-level3} shows the differences related to the simplified versus level 3 MOS models,
where both simulations were performed with ELDO (the only SPICE simulator converging with the level 3 model
without extra capacitor). The simulations differ by details on the conduction period of the NMOS transistor and 
intrinsic diode.
...
The CPU time required to achieve the simulation of $200~\mu s$ ranges between $135~s$ and $410~s$ on a 
...
The CPU time required to achieve the simulation of~$200~\mu s$ with a~50~ps time step
is 110~seconds on a Pentium~4 clocked at 3~GHz.
Results are displayed on figures \ref{fig-buck-inverter-simu-sico-0p2ms-0p05ns} and 
...
The CPU time required to achieve the simulation of~$200~\mu s$ with a~50~ps time step
is~2~hours~31~minutes on a Pentium~4 clocked at 3~GHz. The large increase in CPU time is due to
the lack of sparse matrix techniques in the current implementation of SICONOS. Sparse matrices
are widely used in SPICE simulators since electronics circuits yield frequently sparsity
...
\\
The simulation of the buck with a 2 inverters chain takes 439~s on a Pentium~4 clocked at 3~GHz.
\\
The simulation of the buck with a 50 inverters chain takes 25~minutes on a Pentium~4 clocked at 3~GHz.
\\
...
a precise piecewise linear model yield a too large LCP. Best results could be reached by mixing smooth
...
Results are displayed on figures \ref{fig-buck-inverter-simu-ELDO-0p2ms-0p05ns} and
...
is far from being optimized. Future major enhancements like sparse matrix techniques or non-linear LCP solvers will very
likely reduce the CPU time without impairing accuracy.
\end{description}
...
\hline
 & \textbf{SPICE (ELDO)} & \textbf{PLECS} & \textbf{SICONOS} \\
\hline
Buck converter & 363 s  & 135 s to 410 s & 52 s \\
load : resistor & {\small precise } & {\small very approximate } & {\small slightly approximate }\\
\hline
Buck converter & 439 s & 4 hours 8 min & 110 s \\
load : resistor \& inverters & {\small precise }  & {\small very approximate } & {\small slightly approximate }\\
\hline
\end{tabular}
\newline
\newline
