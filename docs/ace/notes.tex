...
\[y=Cx +D \lambda +e(t)\]
Discretisation:
\[M(x(t_{i+1})-x(t_{i}))=h(\theta A x(t_{i+1}) + (1-\theta) x(t_{i}) +B \lambda (t_{i+1}) +\theta
b(t_{i+1}) + (1-\theta) b(t_{i}))\]
\[Wx(t_{i+1})=hB \lambda (t_{i+1}) + f_{free}\]
with:
\[f_{free}=h(1-\theta) x(t_{i}) + Mx(t_{i})+h \theta b(t_{i+1}) + h(1-\theta) b(t_{i}) \qquad W=(M-h\theta A)\]
